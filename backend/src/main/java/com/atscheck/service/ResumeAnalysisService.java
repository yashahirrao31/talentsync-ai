 ...
 * Parse → Score (AtsScoreService) → Report (AtsReportService) → Save → Return
 *
 * 100% custom algorithm — no external AI API dependencies.
 */
@Service
...
public class ResumeAnalysisService {
    ...
    private final AtsReportService atsReportService;   // ← Our custom engine
    ...
    public ResumeAnalysisResponse analyze(
        ...
        try {
            // Step 1: Extract text (MultipartFile stream can only be read once)
            ...
            // Step 2: Rule-based ATS scoring (7 weighted categories)
            ...
            // Step 3: Custom report generation (no Gemini — 100% our own algorithm)
            log.info("Generating report with custom ATS engine...");
            GeminiReportService.GeminiReport report = atsReportService.generateReport(
                    resumeText, jobDescription, totalScore);
            log.info("Custom report generated: verdict={}", report.verdict);

            // Step 4: Build response
            ResumeAnalysisResponse response = buildResponse(
                    totalScore, scoreBreakdown, report,
                    file.getOriginalFilename(), jobTitle);

            // Step 5: Upload to S3 (non-critical — analysis already complete)
            String s3Key = null;
            try {
    ...
    private ResumeAnalysisResponse buildResponse(
            ...
            GeminiReportService.GeminiReport report,
            String filename,
            String jobTitle
    ) {
        ...
        resp.setStrengths(report.strengths);
        resp.setWeaknesses(report.weaknesses);
        resp.setMissingKeywords(report.missingKeywords);
        resp.setRecommendations(report.recommendations);
        resp.setSectionFeedback(report.sectionFeedback);
        resp.setRewriteSuggestions(report.rewriteSuggestions);
        resp.setVerdict(report.verdict);
        resp.setOverallTips(report.overallTips);
